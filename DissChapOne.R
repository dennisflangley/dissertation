# Title ------------------------------------------------------------------------
##
## name: DissertationChapOne.R
## author: Dennis F Langley
## date: 2018-04-06
## what: Dissertation Chapter One
...
## Merge FSU and Common Content by ID
{
  d <- clean_fsu  %>%
    ...
    as.data.frame()
  d<-subset(d, drop==0, select = -drop)
}

## Clear data sets used to process; keep only the new `d` data frame
keep(d, ggplot_missing, percent_missing, sure = TRUE)

# Data Analysis ----------------------------------------------------------------

## Keep only the variables used in this analysis

d.mod <- subset(d, select = c(id, cces.pri.es, cces.pri.ae, fsu.pri.es, fsu.pri.ae,
                              psi, isi, female, minority, married,
                              party.strength, ideo.strength, partyagree,
                              church_freq, interest, income, educ_cat))

## Fix Missingness
apply(d.mod,2,percent_missing)
ggplot_missing(d.mod)

tempdata <- mice(d.mod, m = 5, maxit = 50, meth = "rf", 
                 seed = 32308, print = FALSE)

mice_output <- complete(tempdata)

### Check distribution of old and imputed variables
### they look the same, so imputation didn't change much
ggplot(d.mod, aes(x=isi)) +
  geom_histogram(color="black", fill=rgb(0.8, 0.2, 0.2, 0.9)) +
  theme_few()+
  ggtitle("isi: Original Data")

ggplot(mice_output, aes(x=isi)) +
  geom_histogram(color="black", fill=rgb(0.6, 0.2, 0.2, 0.5)) +
  theme_few()+
  ggtitle("isi: MICE")

### fill imputed variables into d.mod data set

d.mod <- complete(tempdata)
d.mod$psi[d.mod$party.strength == 0] <- 0 
d.mod$isi[d.mod$ideo.strength == 0] <- 0
keep(d.mod, sure=TRUE)



## Data Visualization

plot(jitter(d.mod$party.strength, 0.5), jitter(d.mod$psi, 1),
     xlab = "Party ID Strength", ylab = "Partisan Social Identity",
     main = "Jitter Plot of PID Strength and Partisan Social Identity")

plot(jitter(d.mod$ideo.strength, 0.5), jitter(d.mod$isi, 1),
...
### Main Results
mod.cces.es1 <- glm(cces.pri.es ~ female + minority + married + party.strength + 
...
## lrtests, showing improved fit
lr.es <- lrtest(mod.cces.es1, mod.cces.es2)
lr.ae <- lrtest(mod.cces.ae1, mod.cces.ae2)
...
### Shows that results are largely driven by Democrats
...
# Misc -------------------------------------------------------------------------
## Extra Models, not shown in paper

## Using question from FSU module about primary candidate
